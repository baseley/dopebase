...
function ActionsItemView({ data }: { data: any }) {
  ...
  const handleView = (item: TicketMessage) => {
    router.push(`./view?id=${item.id}`)
  }

  const handleEdit = (item: TicketMessage) => {
    router.push(`./update?id=${item.id}`)
  }

  const handleDelete = async (item: TicketMessage) => {
    if (window.confirm("Are you sure you want to delete this ticket message?")) {
      setIsDeleting(true)
      try {
        ...
        alert("Failed to delete ticket message. Please try again.")
      } finally {
        setIsDeleting(false)
      }
    }
...
function TicketMessagesListView() {
  ...
  const columns = useMemo<ColumnDef<TicketMessage>[]>(
    () => [
      {
        id: "sender_email",
        header: "Sender Email",
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          const isOP = getValue() as boolean
          return (
            <div className="flex items-center justify-center">
              <div
                style={{
                  ...
                  height: "24px",
                  borderRadius: "50%",
                  backgroundColor: isOP ? theme.colors.feedback.success : theme.colors.surface.tertiary,
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                }}
              >
                <User size={12} color={isOP ? "#fff" : theme.colors.text.secondary} />
              </div>
            </div>
          )
        },
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <div className="flex items-center">
              <MessageSquare size={16} className="mr-2 flex-shrink-0" style={{ color: theme.colors.text.tertiary }} />
              <div
                style={{
                  maxWidth: "400px",
                  overflow: "hidden",
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(baseAPIURL + "ticket_messages/list", config)
        ...
        const cleanData = data.map((item: any) => ({
          ...
          from_original_poster: item.from_original_poster || false,
        ...
        setError(error instanceof Error ? error.message : "Failed to load ticket messages")
      } finally {
        setIsLoading(false)
      }
    }
      ...
      <div style={theme.listView.card}>
        <div style={theme.listView.cardHeader}>
          <h1 style={theme.listView.title}>
            <Mail size={24} style={{ color: theme.colors.accent.primary, marginRight: "0.5rem" }} />
            Ticket Messages
          </h1>
          <button
                          ...
                          >
                            <div style={theme.listView.emptyContainer}>
                              <div style={theme.listView.emptyIconContainer}>
                                <Mail size={32} style={{ color: theme.colors.text.tertiary }} />
                              </div>
                              <div style={theme.listView.emptyTitle}>No messages found</div>
                              <div style={theme.listView.emptyMessage}>
                                Try adjusting your search or create a new message
                              </div>
                            </div>
                          </td>
                        </tr>
