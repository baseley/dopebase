...
function ActionsItemView({ data }: { data: any }) {
  ...
  const handleView = (item: TicketTag) => {
    router.push(`./view?id=${item.id}`)
  }

  const handleEdit = (item: TicketTag) => {
    router.push(`./update?id=${item.id}`)
  }

  const handleDelete = async (item: TicketTag) => {
    if (window.confirm("Are you sure you want to delete this ticket tag?")) {
      setIsDeleting(true)
      try {
        ...
        console.error("Error deleting ticket tag:", error)
        alert("Failed to delete ticket tag. Please try again.")
      } finally {
        setIsDeleting(false)
      }
    }
...
function TicketTagsListView() {
  ...
  const columns = useMemo<ColumnDef<TicketTag>[]>(
    () => [
      {
        id: "name",
        header: "Name",
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <div className="flex items-center">
              <Tag size={16} className="mr-2" style={{ color: theme.colors.text.tertiary }} />
              <span style={{ color: theme.colors.text.primary }}>{name}</span>
            </div>
          )
        },
      },
            ...
            <div className="flex items-center justify-center">
              <div
                style={{
                  ...
                  height: "24px",
                  borderRadius: "50%",
                  backgroundColor: published ? theme.colors.feedback.success : theme.colors.surface.tertiary,
                  display: "flex",
                  alignItems: "center",
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(baseAPIURL + "ticket_tags/list", config)
        ...
        setError(error instanceof Error ? error.message : "Failed to load ticket tags")
      } finally {
        setIsLoading(false)
      }
    }
                          ...
                          >
                            <div style={theme.listView.emptyContainer}>
                              <div style={theme.listView.emptyIconContainer}>
                                <Tag size={32} style={{ color: theme.colors.text.tertiary }} />
                              </div>
